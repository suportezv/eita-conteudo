...
- Modelos de TTS disponíveis nesta chave: `eleven_multilingual_v2` e **`eleven_v3`** (mais expressivo; `stability` só aceita 0.0 Creative / 0.5 Natural / 1.0 Robust e **não aceita `previous_text`/`next_text`**, devolve 400 `unsupported_model`). Receitas em comparação na Bancada de Voz (artifact 0230002f-d877-467c-9a60-1fe083c2acd7): A = v3 stability 0.5; B = v3 stability 0.0 (a mais humana); C = multilingual_v2 stability 0.45 + `style` 0.55 + `use_speaker_boost`.
- **Pronúncia do nome da marca em TTS**: escrever sempre **"Eita" em caixa mista**. Teste de 2026-09-08 contra Êita, Eíta, Êitcha e EITA: todas piores (viram "Eta", "Ita", "Aitya"). Ponto frágil conhecido: **"a Eita"** no início de frase, onde o artigo engole o ditongo e vira "a Eta" (no v3 stability 0.5 aconteceu em 6 de 7 gerações). **Validar toda geração por transcrição Scribe** conferindo que `\beita\b` aparece o número esperado de vezes e que não há "eta"/"ita"; regerar o bloco até passar. O próprio Scribe é instável nesse fonema, então em bloco crítico exigir duas leituras limpas. Script de referência: `refaz.py` do scratchpad da promo anual (gera, apara, transcreve, valida, repete). Entrega das pílulas: zip espelhando `Eita\video\PILULAS` local da equipe (hooks em `audio/NN.mp3`, CTA em `NN/cta-NN.mp3`); aprovação de áudio pela Bancada de Voz (artifact 41a7f720-74b1-43c8-bc1e-e4536bfa7c86).
