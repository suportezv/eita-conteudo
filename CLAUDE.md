...
- OpenAI e Gemini: chaves no environment (`OPENAI_API_KEY`, `GEMINI_API_KEY`), ambas testadas com geração real em set/2026. Servem para texto, transcrição (`gpt-4o-transcribe-diarize`) e imagem, quando a ElevenLabs ou o Kairogen não forem o caminho.
...
- Gemini: `gemini-2.5-flash-lite` responde 404 ("no longer available to new users"). Usar `gemini-3.5-flash-lite`.
- **Nenhum CDN de render passa no environment**: `cdn.jsdelivr.net` e `remotion.media` devolvem 403 no CONNECT do agent proxy. Só `registry.npmjs.org` está liberado, então toda lib de render entra vendorizada via npm. Sintoma no hyperframes: o render captura os frames e aborta em `sub_timeline_script_failure` (GSAP). Sintoma no remotion: 403 ao baixar o Chrome Headless Shell.
- Hyperframes e remotion compartilham o mesmo Chrome Headless Shell, baixado por `npx hyperframes browser ensure` (o download do remotion é o bloqueado). O `remotion.config.ts` resolve esse caminho em tempo de execução; `REMOTION_BROWSER_EXECUTABLE` sobrepõe.
- Projeto hyperframes novo: rodar `bash scripts/vendor-gsap.sh <projeto>` logo depois do `hyperframes init`, antes do primeiro render. O script copia `assets/vendor/gsap.min.js` para `<projeto>/vendor/` e reescreve o `<script src>` do HTML.
- Remotion mora em `/workspace/remotion-studio` (fora do repo, como as outras ferramentas). É o segundo motor de vídeo, em React; o hyperframes continua sendo o padrão. **Licença**: o remotion é source-available e cobra licença de empresa acima do porte mínimo, então confirmar em remotion.pro antes de usar em entrega comercial.
