...
- TTS com a voz da Ana (receitas validadas na leva de pílulas, 2026-08-19): hooks com stability 0.5/similarity 0.8; para entrega mais emocional, stability 0.35 a 0.4 ("variante expressiva", aprovada na pílula 16). **CTAs sempre na receita calma**: speed 0.9, stability 0.65, ponto final em vez de vírgula entre as frases e `<break/>` entre elas (senão sai "metralhadora"). **Anti-corte do final**: o ElevenLabs trunca a última palavra às vezes; sempre fechar o texto com `<break time="0.6s"/>` depois do ponto final. Conferir início e fim por transcrição Scribe dos primeiros/últimos 3s quando o áudio for aprovado sem audição humana. Pausas entre frases dramáticas: 0.4 a 0.5s nas tags do roteiro.
- TTS de texto longo (>30s; receitas do reels promo Setembro Amarelo, 2026-09-01, revisadas em 2026-09-08): **NÃO usar tags `<break>` no meio de texto longo** (geram ruído/artefato, deixam a fala arrastada e, no fim do áudio, balbucio alucinado em outra língua). Montar as pausas como silêncio real no ffmpeg (concat demuxer, gaps de 0.45 a 0.6s), aparando as bordas de cada trecho com `silenceremove` dos dois lados. **NÃO segmentar frase a frase e NÃO forçar `speed`**: as duas coisas foram reprovadas pela equipe em 2026-09-08 ("robótico, o tom muda no meio da frase, sem emoção"). Frase isolada faz a voz recomeçar a prosódia do zero e `speed` diferente de 1.0 mecaniza a fala. **Gerar em blocos longos (3 a 4 frases, um bloco por batida do roteiro), velocidade natural.** Números por extenso ("cinquenta por cento", "doze meses").
- **NUNCA usar `eleven_v3` com a voz da Ana.** Testado em 2026-09-08: o v3 é mais expressivo, mas **não preserva a identidade da voz clonada** (veredito da equipe ouvindo o comparativo: "A e B não são a voz da Ana"). O modelo do estúdio é e continua sendo `eleven_multilingual_v2`. (Outras limitações do v3, se algum dia servir para voz não clonada: `stability` só aceita 0.0/0.5/1.0 e ele rejeita `previous_text`/`next_text` com 400 `unsupported_model`.)
- **Receita aprovada para locução longa (2026-09-08, promo anual)**: `eleven_multilingual_v2`, stability 0.45, similarity 0.8, **`style` 0.55**, `use_speaker_boost` true, **`speed` 1.0**. O `style` alto é o que traz emoção sem descaracterizar a voz. Usar `previous_text`/`next_text` entre os blocos (o multilingual_v2 aceita, e isso costura a prosódia na emenda). Manter **os mesmos parâmetros em todos os blocos da mesma peça**: regerar um bloco com `style` diferente cria um degrau de timbre audível na emenda (foi o defeito que a equipe pegou aos 0:28 da primeira montagem).
- **Bloco curto de TTS (menos de ~10s) é o inimigo da naturalidade** (pílulas de outubro, 2026-09-10): sem embalo a voz sai chapada, gagueja e às vezes inventa um som fantasma na entrada. Tentativa de contornar gravando a pílula inteira e recortando pela marcação de palavras do Scribe (`continuo.py`) resolveu 4 dos 6 casos, mas piorou 2: NÃO é bala de prata. **O que a validação automática NÃO vê: entrega.** Medi melodia (variação de F0 em semitons) e ênfase das tomadas aprovadas contra as reprovadas pela equipe e as duas populações se sobrepõem (`qualidade.py`): não existe corte numérico que separe "vivo" de "robótico". Portanto, para bloco que não acerta na primeira: gerar 4 tomadas com temperos diferentes (`opcoes.py`: stability 0.30/0.45/0.60/0.50 x style 0.60/0.55/0.70/0.40) e **deixar a equipe escolher de ouvido**, em vez de gastar rodadas adivinhando.
- **Pronúncia do nome da marca em TTS**: escrever sempre **"Eita" em caixa mista**. Testes de 2026-09-08 contra Êita, Eíta, Êitcha, Eyta, EITA e "Heita": todas piores (viram "Eta", "Ita", "Aitya", "Reita"). **O ponto frágil é o artigo colado no nome** ("com a Eita", "A Eita te escuta"): o "a" engole o ditongo e sai "Aita" ou "Eta". Não existe grafia que resolva e subir a estabilidade só melhora a taxa (2 em 3 com 0.65), então a solução é **validar e regerar**: transcrever cada bloco com Scribe, exigir que `\beita\b` apareça o número esperado de vezes e que não haja "eta|aeta|aita|ita", e repetir a geração até passar. O próprio Scribe é instável nesse fonema, então **exigir duas leituras limpas seguidas** no bloco que contém o nome. Script pronto: `projects/promo-anual-setembro/fixa_c.py` (gera com contexto, apara, transcreve duas vezes, valida, repete). Entrega das pílulas: zip espelhando `Eita\video\PILULAS` local da equipe (hooks em `audio/NN.mp3`, CTA em `NN/cta-NN.mp3`); aprovação de áudio pela Bancada de Voz (artifact 41a7f720-74b1-43c8-bc1e-e4536bfa7c86).
