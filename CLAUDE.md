...
- TTS de texto longo (>30s; receitas do reels promo Setembro Amarelo, 2026-09-01): **NÃO usar tags `<break>` no meio de texto longo** (geram ruído/artefato, deixam a fala arrastada e, no fim do áudio, balbucio alucinado em outra língua). Em vez disso: gerar por segmentos (1 parágrafo cada) com `previous_text`/`next_text` para continuidade de prosódia, aparar as bordas de silêncio de cada segmento (`silenceremove` dos dois lados) e montar as pausas como silêncio real no ffmpeg (concat demuxer, gaps de 0.45/0.55s). QA por Scribe segmento a segmento pega gagueira e pronúncia errada (regerar só o segmento ruim). **Grafia "Eita" (caixa mista) nos roteiros de TTS, nunca "EITA"**: caixa alta sai como sigla ou interjeição com entonação errada. Números por extenso ("cinquenta por cento", "doze meses"). speed 1.05 com stability 0.35-0.4 dá ritmo humano sem correria. Entrega das pílulas: zip espelhando `Eita\video\PILULAS` local da equipe (hooks em `audio/NN.mp3`, CTA em `NN/cta-NN.mp3`); aprovação de áudio pela Bancada de Voz (artifact 41a7f720-74b1-43c8-bc1e-e4536bfa7c86).
